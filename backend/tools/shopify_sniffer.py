    ...
    if len(orders):
        for order in orders:
            ...
            if user_client_id is None:
                u = User(
                    first_name=order.to_dict()['billing_address']['first_name'], last_name=order.to_dict()['billing_address']['last_name'], cellphone=order.to_dict()['billing_address'].get('phone', 1234).replace(' ', ''), password_hash=''
                )
                db.session.add(u)
                db.session.commit()
                ...
                client_id = db.session.query(Client.id).filter_by(
                    user_id=user_client_id).first()[0]
            print('Client_ID {0}'.format(client_id))
            order_ = Order(
                seller_id=seller_id, client_id=client_id, date=order.to_dict()['updated_at'], status=0, order_number=order.to_dict()['order_number'], total=order.to_dict()['total_price'], tax=order.to_dict()['total_tax'], shipping=order.to_dict()['shipping_lines'][0]['price']
            )
            print('Saving order: ')
            print(order_)
